...
class Groups_Emails {
	...
	/**
	 * Add the hooks when user joines or leaves a group
	 */
	...
	public static function groups_created_user_group( $user_id, $group_id ) {
		if ( class_exists( 'Groups_Group' ) ) {
			...
			$excluded_groups = apply_filters( 'groups_emails_added_user_excluded_groups', array() );

			if ( $group && $user_info ) {
				if ( !in_array( $group_id, $excluded_groups ) ) {
	...
	public static function groups_deleted_user_group( $user_id, $group_id ) {
		if ( class_exists( 'Groups_Group' ) ) {
			$group = new Groups_Group( $group_id );
			$user_info = get_userdata( $user_id );
			$excluded_groups = apply_filters( 'groups_emails_deleted_user_excluded_groups', array() );
